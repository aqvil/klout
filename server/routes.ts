...
import { 
  insertPlayerSchema, insertPlayerStatsSchema, insertScoreSchema,
  insertPlayerFollowSchema, insertPlayerCommentSchema, insertPlayerRatingSchema,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/players/:id/follow", requireAuth, async (req, res) => {
    try {
      const playerId = parseInt(req.params.id);
      if (isNaN(playerId)) {
        return res.status(400).json({ message: "Invalid player ID" });
  ...
  app.delete("/api/players/:id/follow", requireAuth, async (req, res) => {
    try {
      const playerId = parseInt(req.params.id);
      if (isNaN(playerId)) {
        return res.status(400).json({ message: "Invalid player ID" });
  ...
  app.get("/api/players/:id/followers", async (req, res) => {
    try {
      const playerId = parseInt(req.params.id);
      if (isNaN(playerId)) {
        return res.status(400).json({ message: "Invalid player ID" });
  ...
  app.get("/api/players/:id/following-status", requireAuth, async (req, res) => {
    try {
      const playerId = parseInt(req.params.id);
      if (isNaN(playerId)) {
        return res.status(400).json({ message: "Invalid player ID" });
  ...
  app.get("/api/players/:id/comments", async (req, res) => {
    try {
      ...
      const comments = await storage.getPlayerComments(playerId);
      res.json(comments);
    } catch (error) {
      res.status(500).json({ message: "Failed to get player comments" });
    }
  });
  
  app.post("/api/players/:id/comments", requireAuth, async (req, res) => {
    try {
      const playerId = parseInt(req.params.id);
      if (isNaN(playerId)) {
        return res.status(400).json({ message: "Invalid player ID" });
      ...
      // Validate and parse comment data
      ...
      const comment = await storage.createPlayerComment(commentData);
  ...
  app.delete("/api/comments/:id", requireAuth, async (req, res) => {
    try {
      ...
      const success = await storage.deletePlayerComment(commentId, userId);
      if (!success) {
  ...
  app.get("/api/players/:id/ratings", async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to get player ratings" });
    }
  });
  ...
  app.post("/api/players/:id/rate", requireAuth, async (req, res) => {
    try {
      const playerId = parseInt(req.params.id);
      if (isNaN(playerId)) {
        return res.status(400).json({ message: "Invalid player ID" });
      ...
      const playerRating = await storage.createOrUpdatePlayerRating(ratingData);
      
      res.status(201).json(playerRating);
    } catch (error) {
      if (error instanceof ZodError) {
        const validationError = fromZodError(error);
        return res.status(400).json({ message: validationError.message });
      }
      res.status(500).json({ message: "Failed to rate player" });
    }
  });
  
  app.get("/api/players/:id/user-rating", requireAuth, async (req, res) => {
    try {
      const playerId = parseInt(req.params.id);
      if (isNaN(playerId)) {
        return res.status(400).json({ message: "Invalid player ID" });
      ...
      const rating = await storage.getUserRating(userId, playerId);
      if (!rating) {
